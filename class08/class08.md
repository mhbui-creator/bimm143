...
The goal of this mini-project is for you to explore a complete analysos
...
Make sure we don’t accidentally include this in our analysis, let’s
...
We need to scale before PCA with the `scale=TRUE` argument to
`prcomp()`.
...
The first principal component captures 0.4427, which is 44.27% of the
total variance.
...
This plot contains alot of labels and information about relationships
between variables and samples, so it is not easy to understand at first
glance from the overplotting and label clutter.
...
The loading vector for concave.points_mean in PC1 is approximately
\[insert value\], indicating that this feature has a strong positive
contribution to the first principal component. This means it plays an
important role in explaining the variation captured by PC1. However, it
is not the largest contributor, as other features such as area_mean and
perimeter_mean have even larger loadings. These features tend to
dominate PC1 because they are strongly associated with overall tumor
size and shape as these features are more likely to dominate PC1.

## Hierarchical Clustering

> Q12. Which method gives your favorite results for the same data.dist
> dataset? Explain your reasoning. *or* How well does the newly created
> model with four clusters separate out the two diagnoses?

The ward.D2 method gives the most meaningful results since it minimizes
within the cluster variance, this produces compact and well separated
clusters. In contrast to methods like single linkage, which can create
chaining and complete linkage, which can be overly strict, ward.D2
provides clearer and more interpretable groupings. This makes it easier
to identify logical structure in the data.
...
The hierarchical clustering model without PCA does not separate the
diagnoses as well as the PCA based model. Even though some clusters are
enriched for malignant or benign samples, there is more mixing and
several small, less meaningful clusters. Hence, the separation is weaker
and less clear compared to clustering on the principal components
overall.
...
Patient 2 should be prioritized for follow up since it clusters with the
malignant group in the PCA plot. This suggests it has characteristics
more similar to malignant samples. Whereas with Patient 1, it clusters
with the benign group and as a result is a lower priority.
