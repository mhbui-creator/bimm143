...
> **N.B.** We never run `instal.packages()` in our quarto doc (we run it
> once only in our R console) as it would re-install the package every
...
- the **geom** layer () (how you want the plot to be drawn, e.g. points,
  lines, etc.)
...
line fit to the data and turn off the “gray” area region.
...
> Q. First use the **dplyr** package and use the `filfter()` function
> from that package to extract the year 2007.
...
> Q. Make a histogram of lifeExp colored by continent (try using fill =
> or `col=continent` or `col=continent`)
