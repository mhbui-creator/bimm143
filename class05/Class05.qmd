...
> **N.B.** We never run `instal.packages()` in our quarto doc (we run it once only in our R console) as it would re-install the package every time we render our quarto report.
...
> Q. First use the **dplyr** package and use the `filfter()` function from that package to extract the year 2007.
